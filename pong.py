...
class PongGame:
    ...
    def draw(self):
        ...
        if not self.training or (self.training and self.visualize_training):
    ...
    def run(self):
        while self.running:
            ...
            if (keys[pygame.K_w] or keys[pygame.K_UP]) and self.left_paddle.top > 0:
                self.left_paddle.y -= PADDLE_SPEED
            if (keys[pygame.K_s] or keys[pygame.K_DOWN]) and self.left_paddle.bottom < SCREEN_HEIGHT:
                self.left_paddle.y += PADDLE_SPEED

            state = self.get_state()
            action = self.agent.choose_action(state)
            if action == -1 and self.right_paddle.top > 0:
                self.right_paddle.y -= PADDLE_SPEED
            elif action == 1 and self.right_paddle.bottom < SCREEN_HEIGHT:
                self.right_paddle.y += PADDLE_SPEED
...
def train_agent(episodes=15000, visualize=False, difficulty="medium"):
    ...
    for episode in range(episodes):
        ...
        while not episode_over:
            ...
            if game.left_paddle.centery < game.ball.centery and game.left_paddle.bottom < SCREEN_HEIGHT:
                game.left_paddle.y += PADDLE_SPEED
            elif game.left_paddle.centery > game.ball.centery and game.left_paddle.top > 0:
                game.left_paddle.y -= PADDLE_SPEED

            state = game.get_state()
            action = game.agent.choose_action(state)
            if action == -1 and game.right_paddle.top > 0:
                game.right_paddle.y -= PADDLE_SPEED
            elif action == 1 and game.right_paddle.bottom < SCREEN_HEIGHT:
                game.right_paddle.y += PADDLE_SPEED

            game.ball.x += game.ball_vel[0]
            game.ball.y += game.ball_vel[1]

            if game.ball.top <= 0 or game.ball.bottom >= SCREEN_HEIGHT:
            ...
            if game.ball.x < 0 or game.ball.x > SCREEN_WIDTH:
                episode_over = True
...
if __name__ == "__main__":
    ...
    # "train_easy", "train_medium", "train_hard" for training without visualization
    # "train_easy_visual", "train_medium_visual", "train_hard_visual" for training with visualization
